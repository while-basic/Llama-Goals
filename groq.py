...
console.print(f"GROQ_API_KEY is {'set' if GROQ_API_KEY else 'not set'}")
...
def print_welcome():
    welcome_text = Text("🥅 Welcome to Groq Goals 🥅", style="bold magenta")
...
def ask_clarifying_questions(user_goal: str):
    ...
    Your task is to respond with exactly 5 questions to help better understand their goal. MAKE SURE THE QUESTIONS AREN'T DUMB AND GENUINELY HELP YOU BETTER UNDERSTAND HOW YOU ENHANCE THEIR GOAL. You never need to ask when they need something completed as the answer will always be ASAP.
